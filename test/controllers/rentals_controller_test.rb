...
class RentalsControllerTest < ActionDispatch::IntegrationTest
  describe "check-out" do
    it "associates a movie with a customer" do
      ...
      post check_out_url(id: movie.id), params: {
    ...
    it "requires a valid movie id" do
      movie_id = Movie.last.id + 1
      post check_out_url(id: movie_id), params: {
    ...
    it "requires a valid customer ID" do
      ...
      post check_out_url(id: movies(:one).id), params: {
  ...
  describe "check-in" do
    ...
    it "marks a rental complete" do
      post check_in_url(id: @rental.movie.id), params: {
    ...
    it "requires a valid movie id" do
      movie_id = Movie.last.id + 1
      post check_in_url(id: movie_id), params: {
        customer_id: @rental.customer.id
      }
      assert_response :not_found
      data = JSON.parse @response.body
    ...
    it "requires there to be a rental for that customer-movie pair" do
      post check_in_url(id: movies(:two).id), params: {
